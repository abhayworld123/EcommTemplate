...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create social network slider config' },
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.posts !== undefined) config.posts = body.posts;
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to update social network slider config' },
      { status: error instanceof Error && error.message.includes('Unauthorized') ? 403 : 500 }
    );
  }
}
